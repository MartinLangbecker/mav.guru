...
const calendar = (api, data) => {
  ...
  for (const week of cal) {
    ...
    for (const day of week) {
      ...
      if (day.past || !day.price || !day.duration) {
        days.push(
          h('td', { class: 'cell empty' + dayClass }, [
            h('span.date', date),
        ...
        days.push(
          h('td', { class: (day.cheapest ? 'cheapest' : '') + dayClass }, [
  ...
  return h('table#calendar', [
    h('thead', [
      h('tr', [
        h('th', [h('span.dayLong', 'Montag'), h('span.dayShort', 'Mo')]),
        h('th', [h('span.dayLong', 'Dienstag'), h('span.dayShort', 'Di')]),
        h('th', [h('span.dayLong', 'Mittwoch'), h('span.dayShort', 'Mi')]),
        h('th', [h('span.dayLong', 'Donnerstag'), h('span.dayShort', 'Do')]),
        h('th', [h('span.dayLong', 'Freitag'), h('span.dayShort', 'Fr')]),
        h('th', [h('span.dayLong', 'Samstag'), h('span.dayShort', 'Sa')]),
        h('th', [h('span.dayLong', 'Sonntag'), h('span.dayShort', 'So')]),
      ]),
    ]),
    h('tbody', weeks),
  ]);
...
const dayURL = (api, data, day) => {
  ...
  return `./day?origin=${data.input.origin.name}&destination=${
    data.input.destination.name
  }&${api.options.url(data.input).join('&')}&date=${date}`;
};
...
const moreLink = (api, data) => {
  ...
  return [
    h(
      ...
      {
        id: 'later',
        href: `./calendar?origin=${data.input.origin.name}&destination=${
          data.input.destination.name
        }&${api.options
          .url(data.input)
          .join('&')}&weeks=${weeks}&submit=Y#later`,
      },
      'Mehr anzeigen...'
    ),
  ];
