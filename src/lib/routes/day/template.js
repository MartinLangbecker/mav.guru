...
const parseJourney = (api, params) => (journey) => {
  ...
  if (formattedDuration.split(':').length <= 1)
    formattedDuration = '0:' + formattedDuration;
  return {
    departure: moment(journey.legs[0].departure)
      .tz(api.settings.timezone)
      .format('HH:mm'),
    arrival: moment(journey.legs[journey.legs.length - 1].arrival)
      .tz(api.settings.timezone)
      .format('HH:mm'),
...
const products = (journey) => {
  if (journey.products && journey.products.length > 0) {
    ...
    return [
      journey.products[0],
      h('span.columnLong', [', ', ...tail(journey.products).join(', ')]),
    ];
  }
  return ['–'];
};
...
const price = (journey) => [
  ...
  h(
    'span.columnShort',
    Math.round(+journey.price.euros + +journey.price.cents / 100) + '€'
  ),
];
...
const journeyTable = (api, data) => {
  ...
  const head = h('thead', [
    h('tr', [
      h('th', [h('span.columnLong', 'Abfahrt'), h('span.columnShort', 'Ab')]),
      h('th', [h('span.columnLong', 'Ankunft'), h('span.columnShort', 'An')]),
      h('th', [h('span.columnLong', 'Dauer'), h('span.columnShort', 'Zeit')]),
      h('th.changes', [
        h('span.columnLong', 'Umstiege'),
        h('span.columnShort', '⇔'),
      ]),
      h('th.columnMiddle', 'Via'),
      h('th', [h('span.columnLong', 'Typen'), h('span.columnShort', 'Typ')]),
      h('th', 'Preis'),
    ]),
  ]);
  const rows = [];
  ...
  for (const journey of journeys) {
    rows.push([
      h('tr', cheapestClass(journey), [
        h('td', [journey.departure, h('span.columnLong', ' Uhr')]),
        h('td', [journey.arrival, h('span.columnLong', ' Uhr')]),
        h('td', [
...
const generateSubTitleDate = (data) => {
  ...
  return [h('span', date.format('DD.MM.YYYY'))];
};
